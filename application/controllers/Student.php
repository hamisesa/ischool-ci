	...
	public function create()
	{
		...
		if($this->form_validation->run() === true) {	
			...
			if($create == true) {
				...
				$validator['messages'] = "Siswa berhasil ditambahkan!";
			}
			else {
				$validator['success'] = false;
				$validator['messages'] = "Siswa gagal ditambahkan!";
			}			
		} 	
		else {			
			$validator['success'] = false;
	...
	*/
	public function getClassSectionTab($classId = null)
	{
		if($classId) {
            ...
            <li role="presentation" class="active"><a href="#classStudent" aria-controls="classStudent" role="tab" data-toggle="tab">Data Siswa</a></li>              
            ';            	
            	$x = 1;
            	foreach ($sectionData as $key => $value) {            	
					$tab['html'] .= '<li role="presentation"><a href="#countSection'.$x.'" aria-controls="countSection" role="tab" data-toggle="tab"> Sesi ('.$value['section_name'].')</a></li>';
					$x++;
				} // /foreach              
            $tab['html'] .= '</ul>
			...
			foreach ($studentData as $key => $value) {
				...
				$button = '<div class="btn-group">
				  ...
				  <ul class="dropdown-menu">			  	
				    ...
				    <li><a href="#" data-toggle="modal" data-target="#removeStudentModal" onclick="removeStudent('.$value['student_id'].')">Hapus</a></li>			    
				  </ul>
				</div>';
	...
	public function updateInfo($studentId = null)
	{
		if($studentId) {
			...
			if($this->form_validation->run() === true) {					
				...
				if($updateInfo == true) {
					$validator['success'] = true;
					$validator['messages'] = "Siswa berhasil diupdate!";
				}
				else {
					$validator['success'] = false;
					$validator['messages'] = "Siswa gagal diupdate!";
				}			
			} 	
			else {			
				$validator['success'] = false;
	...
	public function updatePhoto($studentId = null)
	{
		if($studentId) {
			...
			if(empty($_FILES['editPhoto']['tmp_name'])) {
				$validator['success'] = false;	
				$validator['messages'] = "Belum Pilih photo!";
			} 
			else {
				...
				if($update == true) {
					$validator['success'] = true;	
					$validator['messages'] = "Photo siswa berhasil diupdate!";	
				}
				else {
					$validator['success'] = false;
					$validator['messages'] = "Photo siswa gagal diupdate!";
				}					
			} // /else
			echo json_encode($validator);
		} // /if
