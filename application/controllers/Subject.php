	...
	public function fetchSubjectTable($classId = null)
	{
		if($classId) {
			...
			<div class="pull pull-right">
	  			<button class="btn btn-default" data-toggle="modal" data-target="#addSubjectModal" onclick="addSubject('.$classId.')"><i class="glyphicon glyphicon-plus-sign"></i> Tambah Mata Pelajaran</button>	
		  	</div>
			    		...
			    		foreach ($subjectData as $key => $value) {
			    			...
			    			$button = '<div class="btn-group">
							  ...
							  <ul class="dropdown-menu">
							    ...
							    <li><a type="button" data-toggle="modal" data-target="#removeSubjectModal" onclick="removeSubject('.$value['subject_id'].','.$value['class_id'].')"> <i class="glyphicon glyphicon-trash"></i> Hapus</a></li>		    
							  </ul>
							</div>';
	...
	public function create($classId = null) 
	{
		...
		if($this->form_validation->run() === true) {							
			...
			if($create == true) {
				...
				$validator['messages'] = "Mata Pelajaran berhasil ditambahkan!";
			}
			else {
				$validator['success'] = false;
				$validator['messages'] = "Mata Pelajaran gagal ditambahkan!";
			}			
		} 	
		else {
			$validator['success'] = false;
	...
	public function update($classId = null, $subjectId = null)
	{
		if($classId && $subjectId) {
			...
			if($this->form_validation->run() === true) {							
				...
				if($update == true) {
					$validator['success'] = true;
					$validator['messages'] = "Mata Pelajaran berhasil diupdate!";
				}
				else {
					$validator['success'] = false;
					$validator['messages'] = "Mata Pelajaran gagal diupdate!";
				}			
			} 	
			else {
				$validator['success'] = false;
	...
	*/	
	public function remove($subjectId = null)
	{
		if($subjectId) {
			...
			if($remove === true) {
				$validator['success'] = true;
				$validator['messages'] = "Mata Pelajaran berhasil dihapus!";
			} 
			else{
				$validator['success'] = false;
				$validator['messages'] = "Mata Pelajaran gagal dihapus!";
			}
			echo json_encode($validator);
		}
	}
