...
class Classes extends MY_Controller 
{
	...
	public function create()
	{
		...
		if($this->form_validation->run() === true) {	
			...
			if($create === true) {
				...
				$validator['messages'] = "Kelas berhasil ditambahkan!";
			}
			else {
				$validator['success'] = false;
				$validator['messages'] = "Kelas gagal ditambahkan!";
			}			
		} 	
		else {
			$validator['success'] = false;
	...
	*/
	public function validate_classname()
	{
		...
		if($validate === true) {
			$this->form_validation->set_message('validate_classname', 'Kelas sudah ada!');
			return false;						
		}
		else {
			return true;
			...
			foreach ($classData as $key => $value) {
				...
				<div class="btn-group">
				  ...
				  <ul class="dropdown-menu">
				    ...
				    <li><a type="button" data-toggle="modal" data-target="#removeClassModal" onclick="removeClass('.$value['class_id'].')"> <i class="glyphicon glyphicon-trash"></i> Hapus</a></li>		    
				  </ul>
				</div>';
	...
	public function update($classId = null) 
	{
		if($classId) {
			...
			if($this->form_validation->run() === true) {	
				...
				if($update == true) {
					$validator['success'] = true;
					$validator['messages'] = "Kelas berhasil diupdate!";
				}
				else {
					$validator['success'] = false;
					$validator['messages'] = "Kelas gagal diupdate!";
				}			
			} 	
			else {
				$validator['success'] = false;
	...
	*/
	public function validate_editclassname()
	{
		...
		if($validate === true) {
			$this->form_validation->set_message('validate_editclassname', 'Kelas sudah ada!');
			return false;						
		}
		else {
			return true;
	...
	*/
	public function validate_editnumericname()
	{
		...
		if($validate === true) {
			$this->form_validation->set_message('validate_editnumericname', 'Lokasi Kelas sudah ada!');
			return false;			
		}
		else {
			return true;
	...
	*/
	public function remove($classId = null)
	{
		if($classId) {
			...
			if($remove === true) {
				$validator['success'] = true;
				$validator['messages'] = "Kelas berhasil dihapus!";
			} 
			else{
				$validator['success'] = false;
				$validator['messages'] = "Kelas gagal dihapus!";
			}
			echo json_encode($validator);
		}
	}
