	...
	public function create()
	{
		...
		if($this->form_validation->run() === true) {				
			...
			if($create == true) {
				...
				$validator['messages'] = "Guru Berhasil ditambahkan!";
			}
			else {
				$validator['success'] = false;
				$validator['messages'] = "Guru gagal ditambahkan!";
			}			
		} 	
		else {
			$validator['success'] = false;
				...
				$button = '<!-- Single button -->
					<div class="btn-group">
					  ...
					  <ul class="dropdown-menu">
					    ...
					    <li><a type="button" data-toggle="modal" data-target="#removeTeacherModal" onclick="removeTeacher('.$value['teacher_id'].')"> <i class="glyphicon glyphicon-trash"></i> Hapus</a></li>		    
					  </ul>
					</div>';
	...
	public function updateInfo($teacherId = null)
	{
		if($teacherId) {
			...
			if($this->form_validation->run() === true) {							
				...
				if($updateInfo == true) {
					$validator['success'] = true;
					$validator['messages'] = "Guru berhasil diupdate!";
				}
				else {
					$validator['success'] = false;
					$validator['messages'] = "Guru gagal diupdate!";
				}			
			} 	
			else {
				$validator['success'] = false;
	...
	public function updatePhoto($teacherId = null)
	{
		if($teacherId) {
			...
			if(empty($_FILES['editPhoto']['tmp_name'])) {
				$validator['success'] = false;	
				$validator['messages'] = "Belum Update Photo!";
			} 
			else {
				...
				if($update == true) {
					$validator['success'] = true;	
					$validator['messages'] = "Berhasil update foto guru!";	
				}
				else {
					$validator['success'] = false;
					$validator['messages'] = "Gagal update foto guru!";
				}					
			} // /else
			echo json_encode($validator);
		} // /if
	...
	*/
	public function remove($teacherId = null)
	{
		...
		if($teacherId) {
			...
			if($remove) {
				$validator['success'] = true;
				$validator['messages'] = "Guru berhasil dihapus!";
			} 
			else {
				$validator['success'] = false;
				$validator['messages'] = "Guru gagal dihapus!";	
			} // /else
		} // /if
